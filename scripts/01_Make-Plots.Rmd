...
readRDS("../output/worldwide_consumption/Herrings, sardines, anchovies.rds")
```
...
protein_map <- suppressMessages(
  protein %>%
    ...
    summarize(Reliance = sum(aa_reliance_pct)) %>%
    filter(year == 2019) %>%
    mutate(Reliance = round(sqrt(Reliance), 2)) %>%
    create_map(fill = "Reliance") +
    labs(fill = "AA protein reliance")  # Label for colorbar
)
